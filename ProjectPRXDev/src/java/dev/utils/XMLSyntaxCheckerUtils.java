...
import static dev.utils.SyntaxState.*;

/**
 *
 * @author huynh
 */
public class XMLSyntaxCheckerUtils {

    private Character quote;

    private String convertAttrToXMLFormat(Map<String, String> attributes) {
        if (attributes.isEmpty()) {
            ...
            attributes.entrySet().stream().map((entry) -> {
                ...
                return keyAndValue;
            }).forEachOrdered((keyAndValue) -> {
                builder.append(keyAndValue);
            });
    ...
    public String makeWellformedXMLFromHTML(String src) {
        src = src + " ";
        ...
        for (int i = 0; i < reader.length; i++) {
            ...
            switch (state) {
                ...
                case EQUAL_WAIT:
                    if (isSpace(c)) {
                        //do nothing, just wait for equal character
                    } else if (c == EQ) {
                        state = EQUAL;
                    } else {
                        if (isStartTagChars(c)) {
                ...
                case CLOSE_BRACKET:
                    if (isOpenTag) {
                        ...
                        writer.append(LT)
                                .append(openTagName)
                                .append(convertAttrToXMLFormat(attributes))
                                .append((isEmptyTag ? "/" : ""))
                                .append(GT);
